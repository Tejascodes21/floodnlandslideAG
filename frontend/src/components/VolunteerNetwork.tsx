...
import { HeartHandshake, Compass, Navigation, Search, Sliders, ShieldAlert, CheckCircle, Car, LifeBuoy, Users, Activity, Plus } from 'lucide-react';
...
export default function VolunteerNetwork({ lang }: VolunteerNetworkProps) {
  ...
  // Fetch Missions
  const { data: missions = [], refetch: refetchMissions } = useQuery<RescueMission[]>({
  ...
  useEffect(() => {
    if (!mapRef.current || !volunteerGroupRef.current || !routeGroupRef.current) return;
    ...
    missions.forEach((mission) => {
      if (mission.route && mission.route.length > 1) {
        const polyPoints = mission.route.map(pt => [pt.lat, pt.lon] as L.LatLngTuple);
  ...
  const handleUpdateMissionStatus = (id: number, nextStatus: string) => {
    queryClient.setQueryData(['rescueMissions'], (old: RescueMission[] = []) => 
      old.map(m => m.mission_id === id ? { ...m, status: nextStatus } : m)
    );
  };

  const handleTriggerDispatch = (volId: number) => {
    // Optimistic dispatch state mutation
    queryClient.setQueryData(['volunteers'], (old: Volunteer[] = []) =>
      old.map(v => v.id === volId ? { ...v, active: true } : v)
    );

    // Append to missions deck
    const vol = volunteers.find(v => v.id === volId);
    if (vol) {
      const newMission: RescueMission = {
        mission_id: Math.floor(1000 + Math.random() * 9000),
        volunteer_name: vol.full_name,
        vehicle: vol.vehicle_type,
        citizen: "Citizen Trapped Zone B",
        emergency_type: "Evacuation logistics",
        route: [
          { lat: vol.lat, lon: vol.lon },
          { lat: vol.lat + 0.004, lon: vol.lon - 0.003 }
        ],
        status: "Dispatched"
      };

      queryClient.setQueryData(['rescueMissions'], (old: RescueMission[] = []) => [newMission, ...old]);
    }
  };

  const filteredVolunteers = volunteers.filter((vol) => {
    const matchesSearch = vol.full_name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          vol.skills.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesSkill = filterSkill === "all" || vol.skills.toLowerCase().includes(filterSkill.toLowerCase());
    const matchesVehicle = filterVehicle === "all" || vol.vehicle_type.toLowerCase().includes(filterVehicle.toLowerCase());
    return matchesSearch && matchesSkill && matchesVehicle;
  });

  const standbyCount = volunteers.filter(v => !v.active).length;
  const activeMissionsCount = missions.filter(m => m.status !== "Completed" && m.status !== "Resolved").length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-panel p-4 rounded-xl border border-borderDim flex items-center justify-between">
          <div>
            <span className="text-[10px] text-textMuted uppercase font-bold tracking-wider">{t.kpi_standby}</span>
            <span className="text-2xl font-black block mt-1 text-emerald-400">{standbyCount} standby</span>
          </div>
        ...
        <div className="glass-panel p-4 rounded-xl border border-borderDim flex items-center justify-between">
          <div>
            <span className="text-[10px] text-textMuted uppercase font-bold tracking-wider">{t.kpi_active}</span>
            <span className="text-2xl font-black block mt-1 text-blue-400">{activeMissionsCount} active</span>
          </div>
        ...
        <div className="glass-panel p-4 rounded-xl border border-borderDim flex items-center justify-between">
          <div>
            <span className="text-[10px] text-textMuted uppercase font-bold tracking-wider">{t.kpi_sos}</span>
            <span className="text-2xl font-black block mt-1 text-red-500">2 Unresolved</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col h-[520px] glass-panel p-4 rounded-2xl border border-borderDim relative">
          ...
          <div id="volunteer-gis-map" className="flex-1 w-full rounded-xl border border-slate-950 z-10" />
        </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3.5 pr-1">
            {filteredVolunteers.length > 0 ? (
              filteredVolunteers.map((vol) => (
                <div key={vol.id} className="glass-panel p-4 rounded-xl border border-borderDim space-y-2.5 transition-all hover:bg-slate-900/25">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-gray-200 block">{vol.full_name}</span>
                  ...
                  <div className="text-[10px] font-semibold text-textMuted space-y-1">
                    <div>{t.skills}: <span className="text-slate-400">{vol.skills}</span></div>
                    <div>{t.vehicle}: <span className="text-slate-400">{vol.vehicle_type}</span></div>
                  </div>

                  {!vol.active && (
                    <button
                      onClick={() => handleTriggerDispatch(vol.id)}
                      className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-1 px-3 rounded text-[10px] transition-all cursor-pointer font-black uppercase"
                    >
                      {t.dispatch_btn}
                    </button>
                  )}
              ...
              <tbody className="divide-y divide-borderDim/40">
                {missions.map((m) => (
                  <tr key={m.mission_id} className="hover:bg-slate-900/10">
                    <td className="py-3 px-3 font-mono font-bold text-slate-400">#MS-{m.mission_id}</td>
                    <td className="py-3 px-3 text-textMain">{m.volunteer_name} <span className="text-[10px] text-slate-500">({m.vehicle})</span></td>
                    <td className="py-3 px-3 text-textMain">{m.citizen}</td>
                    <td className="py-3 px-3 text-red-400 font-bold">{m.emergency_type}</td>
                    <td className="py-3 px-3 font-mono text-[10px] text-blue-400">
                      {m.route ? `(${m.route[0]?.lat.toFixed(3)}, ${m.route[0]?.lon.toFixed(3)}) -> (${m.route[m.route.length-1]?.lat.toFixed(3)}, ${m.route[m.route.length-1]?.lon.toFixed(3)})` : '--'}
                    </td>
                    ...
                    <td className="py-3 px-3 text-right">
                      {m.status !== 'Completed' && m.status !== 'Resolved' && (
                        <button
                          onClick={() => handleUpdateMissionStatus(m.mission_id, 'Completed')}
                          className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-1 px-3 rounded text-[9px] transition-all cursor-pointer font-black uppercase"
                        >
                          {t.complete_btn}
                        </button>
                      )}
